...
describe('Pipeline Model', () => {
  ...
  describe('create', () => {
    it('should create a pipeline with defaults', () => {
      ...
      expect(pipeline.credentialId).toBeUndefined();
      expect(pipeline.branch).toBe('main');
      expect(pipeline.stages).toEqual([]);
      expect(pipeline.triggers).toEqual({ manual: true, push: false, schedule: undefined });
      expect(pipeline.environment).toEqual([]);
      expect(pipeline.status).toBe('inactive');
      expect(pipeline.enabled).toBe(true);
      expect(pipeline.lastBuildAt).toBeUndefined();
      expect(pipeline.lastBuildStatus).toBeUndefined();
      expect(pipeline.lastCommit).toBeUndefined();
