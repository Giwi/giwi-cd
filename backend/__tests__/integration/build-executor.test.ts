...
jest.mock('../../src/services/GitService', () => {
  const MockGitService = jest.fn().mockImplementation(function() {
    this._workDir = null;
    this.cloneOrPull = jest.fn().mockResolvedValue({ success: true, workDir: '/tmp' });
    this.getWorkspaceDir = jest.fn().mockReturnValue('/tmp');
    this.getLastCommitMessage = jest.fn().mockResolvedValue('Test commit');
  });
  return { __esModule: true, default: MockGitService };
});

describe.skip('BuildExecutor Integration', () => {
  let BuildExecutor;
  let Build;
  let Pipeline;
  let db;
  ...
  beforeAll(() => {
    BuildExecutor = require('../../src/services/BuildExecutor').default;
    Build = require('../../src/models/Build').default;
    Pipeline = require('../../src/models/Pipeline').default;
  ...
  beforeEach(() => {
    ...
    executor.gitService.cloneOrPull.mockResolvedValue({ success: true, workDir: testWorkDir });
    executor.gitService.getWorkspaceDir.mockReturnValue(testWorkDir);
    executor.gitService.getLastCommitMessage.mockResolvedValue('Test commit');
  });
