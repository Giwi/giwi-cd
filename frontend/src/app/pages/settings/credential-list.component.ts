...
@Component({
  ...
  template: `
    <div class="page-header">
      <div>
        <h1 class="page-title">Identifiants</h1>
        <p class="page-subtitle">Gérer les accès aux dépôts Git</p>
      </div>
    ...
    @if (loading()) {
      ...
      <div class="card border-0 shadow-sm">
        <div class="card-body text-center py-5">
          ...
          <h5>Aucun identifiant</h5>
          <p class="text-muted mb-3">Ajoutez des identifiants pour accéder à vos dépôts Git privés</p>
          <a routerLink="/settings/credentials/new" class="btn btn-primary">
            <i class="bi bi-plus-lg me-1"></i> Ajouter un identifiant
          </a>
        </div>
      </div>
    } @else {
...
export class CredentialListComponent implements OnInit {
  ...
  getTypeLabel(type: string): string {
    const labels: Record<string, string> = {
      'username-password': 'Login / Mot de passe',
      'token': 'Token d\'accès',
      'ssh-key': 'Clé SSH'
    };
